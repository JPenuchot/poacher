...
namespace detail {
/// Accepts a generator function that returns a vector of
/// ast_node_ptr_t, and generates its code.
template <auto Generator> constexpr auto vector_codegen() {
  ...
  static_assert(
      ...
      "vector_codegen(): Invalid parameter Generator. Generator "
      "should be a "
...
template <auto Generator> constexpr auto codegen() {
  ...
  using GeneratorReturnType = decltype(Generator());
  static_assert(
      std::is_same_v<GeneratorReturnType, ast_node_ptr_t>,
  ...
  if constexpr (Kind == ast_token_v) {
    ...
    constexpr token_t Token =
        static_cast<ast_token_t const *>(Generator().get())
            ->get_token();
  ...
  // Node is a block
    ...
    constexpr auto VectorGenerator =
      ...
      return std::move(static_cast<ast_block_t *>(Generator().get())
                           ->get_content());
    };
