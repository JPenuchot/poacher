...
/// Parses BF code until the end of the block (or the end of the
/// formula, ie. parse_end), then returns an iterator to the last
/// parsed token or parse_end if the parser has parsed all the tokens.
...
parse_block(token_vec_t::const_iterator parse_begin,
  ...
  for (; parse_begin != parse_end; parse_begin++) {
    // While end bracket: return block content and while block end
    // position
    ...
    // then continue parsing from the end of the block
      ...
      block_content.push_back(std::make_unique<ast_while_t>(
          std::move(while_block_content)));

      parse_begin = while_block_end;
    }

    // Any other token that is not a nop instruction: add it to the
    // AST
    else if (*parse_begin != nop_v) {
      block_content.push_back(ast_node_ptr_t(
          std::make_unique<ast_token_t>(*parse_begin)));
    }
  }
